...
//  : (중요) 여러 개의 요소를 담을 수 있는 데이터 구조를 제공하여 데이터를 저장하고 관리하는 자료구조 class
...
//      : 그냥 당신이 알고 있는 알고리즘에 쓰이는 데이터 저장용 '자료구조' 들이라 생각하면 편함
...
//          : <자료형> 을 의미하는 기호로 일종의 list나 vector와 같은 가변형 동적배열이나, list계열의 linkedList를 생성시 특정 class의 객체배열로 사용하기 위해 입력 자료형을 지정하는 역할을 함 
...
//                    -> 애초부터 템플릿을 활용하여, class나 function을 선언(= 정의)하는 방법까지 알고 싶으면 day18을 참조
...
//              : 컨테이너클래스명<자료형> :: iterator 이터레이터객체명;
...
//           - (컨테이너 class의 맴버변수(= 멤버객체)로서) 이터레이터 객체 선언 형식
//              : 컨테이너클래스명<자료형> :: iterator 이터레이터객체명;
//                 -> (중요) 이를 통해 어떤 컨테이너 클래스 객체 요소의 유사주소값인 이터레이터 반환값을 대입하여 저장 가능 = 컨테이너 클래스 객체의 다음 노드 주소값을 ++, --를 통해 쉽게 구해낼 수 있고 역참조 가능 
           
//                     ex) Container<data>::iterator itr = Container.begin();
//                          -> C++ Iterator 형식으로 저장된 값은 '유사 포인터'형식을 띔 
//                             (= Iterator 클래스의 멤버변수가 유사 포인터이기 때문)
           

//           - 이터레이터 연산 법칙 
//              : 유사 포인터라는 점 -> 포인터 연산법칙과 유사
//         
//              1. 이터레이터가 가르키는 값 참조 역참조 가능                
//                  ex) *itr
//         
//              2. 유사 포인터인 이터레이터는 타 이터레이터 값을 대입 가능
//                  ex) itr1 = itr
//         
//              3. 다른 이터레이터와 그 위치값 비교 가능
//                  ex) tr == itr1, itr != itr2
//         
//              4. 증감 연산자를 통해서도 이터레이터의 위치값을 변경 가능
//                  ex) 증감 연산 : ++itr, itr++
           

//           - 범위 기반 반복문
//              : 반복문 for에 이터레이터를 사용하여, 특정 컨테이너 객체의 모든 요소 접근할 수 있도록 만든 명령어로 타 언어에서는 foreach로 명명
//                 -> 사용하게 되면, for문은 내부적으로 해당 컨테이너 class 객체의 begin()과 end() 멤버 함수를 호출하여 이터레이터의 시작과 끝 위치값을 얻어냄
//                     -> 그렇게 얻은 시작 ~ 끝을 이터레이터의 증감연산자 ++, -- 사용하여 순회
//         
//               - C++에서 범위기반 반복문 형식
//                  : for(컨테이너자료형 객체명 : 컨테이너객체) { '객체명'을 사용한 내용 }
//                     <-> 타언어에서는 ':' -> in으로 변경해서 사용
//         
//                    ex) for (int number : numbers){  std::cout << number << std::endl;  }
