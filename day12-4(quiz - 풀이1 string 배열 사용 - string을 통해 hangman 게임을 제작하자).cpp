...
int main() {
	...
	hangman* game = new hangman(word, sizeof(word) / sizeof(word[0]));
...
void hangman::menu(int& menu) {
	...
	if (menu == PLAY || menu == CHEAT) {
		...
		while (!this->isSolve() && this->lifeCheck() > 0) {

			this->printAnswer(menu);
			...
			// 입력에 따른 결과에 따른 결과 리턴(0 : 정답, 1 : 오답, 2 : 중복)
			int flag = this->guesses(letter);

			if (flag == 0) {
				cout << "     ->> (anouncement!) good answer!" << (!this->isSolve() ? " keep going!" : " complete!!") << endl << endl;
			}else if (flag == 1) {
...
// 알파벳 & 답안 비교 함수 (0 : 정답, 1 : 오답, 2 : 중복)
int hangman::guesses(char letter) {
