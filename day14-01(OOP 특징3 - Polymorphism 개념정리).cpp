...
//    -> 대충 어떤 객체의 속성(변수)이나 기능(함수)이 상황에 따라 여러 가지 형태를 가질 수 있다는 내용이며, 이를 통해 다음과 같은 것들을 가능하게 할 수 있어야 함
//        1. 자식 class는 부모에게서 물려받은 멤버함수를 override(재정의) 할 수 있음
//        2. 부모 class 객체포인터로 자식 class 객체를 통제하고, 관련 멤버함수를 호출도 가능함

//    -> (중요!) 2의 실현을 위해 '컨트롤 클래스'가 필요함.. 그리고 그 방법의 일환으로 부모class객체포인터로 자식 class객체의 위치값을 저장하는 upcasting을 사용함  
//        -> 단! 1, 2의 내용이 완전히 실현되기 위해서는 가상함수(virtual function) 개념이 필요.. 이를 가능하게 하려면 '동적 바운딩'이 가능하게 조치해야 함
