...
class Customer {
	...
	public:
		Customer(string _name, string _cellphone, vector<menu*>* _orderlist) {
			name = _name;
...
int Customer::howMuch() {
	...
	for (int i = 0; i < orderlist->size(); i++) {
		...
		result += orderlist->at(i)->getprice();
	}
...
int main() {
	...
	vector<menu*> orderlist;
	vector<menu*>* orderlistPoint = &orderlist;
