...
//      : 그냥 friend 걸린 대상은 함수의 경우 호출되거나, class 객체의 경우 선언될 시에 자신을 friend로 인정해 준 class 내부를 public 영역마냥 휘젓고 다닌다로 이해하면 됨
// 
...
// 
//            # friend 멤버함수 코드 작성 위치 및 순서 (1 ~ 3이 순차적으로 작성되어야)
//               (1) friend로 규정될 예정인 멤버함수를 포함할 예정인 '외부 class'가 최소한 이름뿐인 선언부라도 먼저 위치해야함
//               (2) 해당 멤버함수를 friend라고 선언할 '해당 class'와 friend로 지정해줄 멤버함수의 선언부까지 작성되어야 함
//               (3) friend 멤버함수의 정의부를 외부영역으로 돌려 위치해두고, 정의부의 내용을 입력함
// 
...
//                      b. 게다가 내부영역 작성을 허가하면 friend 대상인 멤버함수 선언이나, friend 선언 단계랑 겹칠수도 있는 상황
//                         (= 컴파일러 입장에서 논리적 상황파악을 제대로 하기 까다로워서 error룰 뱉을 가능성 높음)
