...
class Customer {
	...
	public:
		...
		Customer(string _name, string _cellphone, menu** _orderlist) {
			name = _name;
...
class coffie : public menu {
	...
	public:
		...
		// 메뉴에 해당되는 녀석을 고객의 menulist 동적객체배열에 추가
		virtual void pickMenu(Customer* customer) {
			customer->getOrderlist()[customer->getOrderNum()] = this;
			customer->addNumOfmenu();
			cout << " ->> " << getMenuName() << "이(가) 메뉴에 추가되었습니다." << endl;
		}
...
class desert : public menu {
	...
	public:
		...
		virtual void pickMenu(Customer* customer) {
			customer->getOrderlist()[customer->getOrderNum()] = this;
			customer->addNumOfmenu();
			cout << " ->> (알림) " << getMenuName() << "이(가) 메뉴에 추가되었습니다!!!" << endl;
		}
...
int main() {
	...
	// 각 메뉴의 데이터를 
	menu* menulist[100] = { NULL };
	menu** menuPointer = menulist;

	Customer* customer = new Customer("이니셜", "010-2222-1111", menuPointer);
